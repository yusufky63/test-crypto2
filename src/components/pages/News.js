...
setLoading(false);
  ...
  return (
    ...
    <ul className="flex justify-center flex-wrap mt-5">
     ...
     {" "}
      <button className="border-blue-600 border p-2 font-medium mx-10" onClick={()=> setPage(page < 0 ? page : (page - 1) )}>Onceki Sayfa {"<"} {page  }</button>
     <span className="">{page}</span>
      <button  className="border-blue-600 border p-2 font-medium mx-10" onClick={()=> setPage(page + 1)}>Sonraki Sayfa {">"} {page + 1}</button>
